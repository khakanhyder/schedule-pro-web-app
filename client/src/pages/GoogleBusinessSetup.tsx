...
import { 
  ...
  ExternalLink, Upload, Clock, Phone, Mail, Edit3, Shield, ArrowLeft
} from "lucide-react";
...
export default function GoogleBusinessSetup() {
  ...
  const [setupMode, setSetupMode] = useState<'choice' | 'create' | 'link' | 'verify'>('choice');
  ...
  const isVerified = (p?: GoogleBusinessProfile) => p?.verificationStatus === "VERIFIED";
  
  const renderVerificationBadge = (p?: GoogleBusinessProfile) => {
    const status = p?.verificationStatus ?? "UNLINKED";
    switch (status) {
      case "VERIFIED":
        return (<Badge className="bg-green-100 text-green-800"><CheckCircle className="h-3 w-3 mr-1"/>Verified</Badge>);
      case "PENDING_VERIFICATION":
        return (<Badge variant="outline" className="text-orange-600 border-orange-200"><AlertCircle className="h-3 w-3 mr-1"/>Pending verification</Badge>);
      case "LINKED_UNVERIFIED":
        return (<Badge variant="outline" className="text-yellow-700 border-yellow-200">Linked — not verified</Badge>);
      case "FAILED":
        return (<Badge variant="outline" className="text-red-700 border-red-200">Verification failed</Badge>);
      default:
        return (<Badge variant="outline" className="text-gray-600 border-gray-200">Not linked</Badge>);
    }
  };
  ...
  const getStepStatus = (stepId: number, profile?: GoogleBusinessProfile) => {
    ...
    switch (stepId) {
      ...
      case 2: return isVerified(profile) ? "completed" : (profile?.googlePlaceId || profile?.oauthConnected) ? "current" : "pending";
      ...
      {!businessProfile ? (
        ...
        <>
          ...
          {setupMode === 'verify' && (
            <Card>
              <CardHeader>
                <CardTitle>Verify with Google</CardTitle>
                <CardDescription>
                  Connect with Google to verify your business profile and enable automatic sync
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="text-center space-y-4">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold mb-2">Google Business Profile Verification</h3>
                    <p className="text-gray-600 mb-4">
                      To ensure your business profile is verified and can receive automatic updates, 
                      you need to authenticate with Google using your business account.
                    </p>
                  </div>
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    <ul className="text-sm text-blue-800 space-y-1 text-left">
                      <li>• Verify ownership of your Google Business Profile</li>
                      <li>• Enable automatic sync of reviews and ratings</li>
                      <li>• Allow photo uploads and business information updates</li>
                      <li>• Receive notifications about customer interactions</li>
                    </ul>
                  </div>
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        ...
                        toast({
                          ...
                          description: "You can verify your profile later in the settings. Your profile will show as 'Not Verified' until then.",
                        });
                        setSetupMode('choice'); // Go back or handle as needed
                      }}
                    >
                      Skip for Now
                    </Button>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {renderVerificationBadge(profile)}
                </div>
              </div>
            </CardHeader>
